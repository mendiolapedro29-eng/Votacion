/**
 * Propuestas políticas asociadas a cada partido político
 * 
 * Este archivo contiene las propuestas de cada partido político
 * que se utilizan para mostrar información en la página de votación.
 */

export const propuestasPorPartido = {
  "Acción Popular": [
    "Democracia participativa y fortalecimiento institucional",
    "Desarrollo económico sostenible",
    "Educación y salud de calidad",
    "Transparencia y lucha contra la corrupción",
  ],
  "Ahora Nación": [
    "Innovación tecnológica y educación superior",
    "Desarrollo científico y tecnológico",
    "Modernización del estado",
    "Inclusión y desarrollo social",
  ],
  "Alianza para el Progreso": [
    "Desarrollo regional y descentralización",
    "Infraestructura y obras públicas",
    "Crecimiento económico",
    "Generación de empleo",
  ],
  "País para Todos": [
    "Entretenimiento y cultura",
    "Desarrollo social",
    "Inclusión",
  ],
  "Avanza País": [
    "Desarrollo económico",
    "Modernización",
    "Crecimiento sostenible",
  ],
  "Integridad Democrática": [
    "Transparencia",
    "Ética política",
    "Fortalecimiento democrático",
  ],
  "Partido Regionalista de Integración Nacional": [
    "Desarrollo regional",
    "Integración nacional",
    "Fortalecimiento de regiones",
  ],
  "Fe en el Perú": [
    "Valores y principios",
    "Desarrollo nacional",
    "Unidad",
  ],
  "Fuerza Popular": [
    "Seguridad ciudadana",
    "Crecimiento económico",
    "Estabilidad política",
    "Fortalecimiento de la familia",
  ],
  "Partido Patriótico del Perú": [
    "Patriotismo y valores nacionales",
    "Desarrollo económico",
    "Seguridad nacional",
  ],
  "Partido Democrático Federal": [
    "Federalismo y descentralización",
    "Democracia participativa",
    "Desarrollo regional",
  ],
  "Partido Morado": [
    "Modernización del estado",
    "Transparencia y anticorrupción",
    "Derechos humanos",
    "Innovación y tecnología",
  ],
  "Juntos por el Perú": [
    "Justicia social e inclusión",
    "Derechos laborales",
    "Protección del medio ambiente",
    "Desarrollo rural y agrario",
  ],
  "Libertad Popular": [
    "Libertad individual",
    "Democracia liberal",
    "Desarrollo económico",
  ],
  "Somos Perú": [
    "Desarrollo local y regional",
    "Obras públicas",
    "Seguridad ciudadana",
    "Apoyo a la pequeña empresa",
  ],
  "Renovación Popular": [
    "Valores cristianos y familia",
    "Seguridad y orden",
    "Desarrollo económico",
    "Lucha contra la corrupción",
  ],
  "Perú Primero": [
    "Desarrollo nacional",
    "Crecimiento económico",
    "Modernización",
  ],
  "Un Camino Diferente": [
    "Cambio y renovación",
    "Desarrollo alternativo",
    "Innovación",
  ],
  "Partido Primero La Gente": [
    "Prioridad a las personas",
    "Desarrollo social",
    "Inclusión",
  ],
  "Partido Ciudadanos por el Perú": [
    "Ciudadanía activa",
    "Participación ciudadana",
    "Desarrollo democrático",
  ],
  "Salvemos al Perú": [
    "Rescate nacional",
    "Unidad",
    "Desarrollo urgente",
  ],
  "Frente de la Esperanza": [
    "Esperanza y cambio",
    "Desarrollo social",
    "Inclusión",
  ],
  "Perú Libre": [
    "Socialismo y justicia social",
    "Nacionalización de recursos",
    "Reforma agraria",
    "Derechos de los trabajadores",
  ],
  "Podemos Perú": [
    "Desarrollo económico",
    "Seguridad ciudadana",
    "Inversión en infraestructura",
    "Generación de empleo",
  ],
  "Cooperación Popular": [
    "Desarrollo social",
    "Educación y salud",
    "Inclusión social",
    "Fortalecimiento democrático",
  ],
  "Sí creo": [
    "Fe y valores",
    "Desarrollo con principios",
    "Unidad nacional",
  ],
  "Partido del Buen Gobierno": [
    "Buen gobierno",
    "Transparencia",
    "Eficiencia",
  ],
  "Partido Demócrata Unido Perú": [
    "Unidad democrática",
    "Desarrollo inclusivo",
    "Fortalecimiento institucional",
  ],
  "Fuerza y Libertad": [
    "Libertad económica",
    "Desarrollo empresarial",
    "Reducción del estado",
  ],
  "Unidad Nacional": [
    "Unidad y reconciliación nacional",
    "Desarrollo económico inclusivo",
    "Fortalecimiento democrático",
    "Justicia y transparencia",
  ],
  "Venceremos": [
    "Victoria y triunfo",
    "Desarrollo nacional",
    "Unidad",
  ],
};
// Función para obtener propuestas por partido
export const obtenerPropuestas = (partidoPolitico) => {
  return propuestasPorPartido[partidoPolitico] || [
    "Desarrollo económico sostenible",
    "Fortalecimiento institucional", 
    "Bienestar social",
    "Seguridad ciudadana",
    "Educación de calidad"
  ];
};

// Función para obtener todos los partidos con propuestas
export const obtenerPartidosConPropuestas = () => {
  return Object.keys(propuestasPorPartido);
};

export default propuestasPorPartido;
